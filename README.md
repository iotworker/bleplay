...
BlePlay 是一个基于 Qt6 Widgets 的桌面 BLE 抓包分析工具，用于离线查看和分析 `.btt`、`btsnoop`、`.pklg` 等日志。它也支持搭配 nRF52840 进行 BLE 实时抓包分析，并可将采集结果保存为 `pcap` 文件。BlePlay 面向蓝牙协议调试、连接过程排查和 GATT 行为观察场景，帮助用户从原始抓包快速定位到可读协议事件、会话上下文和属性结构。

## 功能特性

- 导入 `.btt`、`btsnoop`、`.pklg` 等 BLE/HCI 日志文件
- 支持 nRF52840 实时抓包分析，并可导出为 `pcap`
...
- 使用 nRF52840 进行实时 BLE 抓包、分析并保存为 `pcap`
- 排查配对失败、连接异常、中断断开等问题
...
BlePlay 可以直接连接 nRF52840 嗅探器进行实时抓包，边采集边分析，并在需要时将结果保存为 `pcap`。

基本流程如下：

1. 在主界面打开实时抓包入口，启动 `Start Live Capture`
2. 选择抓包模式
   - `Scan advertising`：扫描广播包
   - `Follow device`：输入目标设备地址后跟踪该设备
3. `Port` 保持空白时可自动识别设备，也可以手动指定端口
4. 如需保存文件，勾选 `Save capture to PCAP`，并指定 `PCAP file`
5. 点击确认后开始采集，抓包结果会实时进入 `Packets`、`Sessions` 和 `GATT Map`
6. 停止抓包后，`pcap` 文件会写入到指定位置
